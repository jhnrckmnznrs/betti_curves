...
use crate::merge_tree_h0::{
    AttachEvent as H0AttachEvent, BoundaryFaceNodes as H0BoundaryFaceNodes,
...
use crate::merge_tree_h2::{
    AttachEvent as H2AttachEvent, BoundaryFaceNodes as H2BoundaryFaceNodes,
...
fn merge_h0_event_streams(
    ...
    while merged.len() < total {
        ...
        if cursor[0] < left_attach.len() {
            consider(0, 0, left_attach[cursor[0]].value);
        }
        if cursor[1] < right_attach.len() {
            consider(1, 0, right_attach[cursor[1]].value);
        }
        if cursor[2] < left_interface.len() {
            consider(2, 1, left_interface[cursor[2]].value);
        }
        if cursor[3] < right_interface.len() {
            consider(3, 1, right_interface[cursor[3]].value);
        }
        if cursor[4] < cross.len() {
            consider(4, 2, cross[cursor[4]].value);
        }

        let (_, _, stream) = best.expect("H0 ordered fan-in lost a non-empty source");
        let event = match stream {
            0 => {
                let source = left_attach[cursor[0]];
                cursor[0] += 1;
                H0Event {
                    value: source.value,
                    kind: H0EventKind::Attach {
                        node: left_offset + source.interface_node,
                        branch: source.branch,
                    },
                }
            }
            1 => {
                let source = right_attach[cursor[1]];
                cursor[1] += 1;
                H0Event {
                    value: source.value,
                    kind: H0EventKind::Attach {
                        node: right_offset + source.interface_node,
                        branch: source.branch,
                    },
                }
            }
            2 => {
                let source = left_interface[cursor[2]];
                cursor[2] += 1;
                H0Event {
                    value: source.value,
                    kind: H0EventKind::Interface {
                        a: left_offset + source.a,
                        b: left_offset + source.b,
                    },
                }
            }
            3 => {
                let source = right_interface[cursor[3]];
                cursor[3] += 1;
                H0Event {
                    value: source.value,
                    kind: H0EventKind::Interface {
                        a: right_offset + source.a,
                        b: right_offset + source.b,
                    },
                }
            }
            4 => {
                let source = cross[cursor[4]];
                cursor[4] += 1;
                H0Event {
                    value: source.value,
...
fn apply_h0_action(
    ...
    match action {
        H0Action::None => {}
        H0Action::Final(merge) => history.push_merge(merge),
...
fn combine_h0(
    left: HierarchicalSummary<SlabH0MergeTreeSummary>,
    right: HierarchicalSummary<SlabH0MergeTreeSummary>,
    ...
    println!(
        "PROFILE_BRANCH_H0_HIER_COMBINE z0={} z1={} pair_nodes={} parent_interface_nodes={} parent_attach={} parent_interface={} cross_retained={} cross_candidates={} setup_seconds={:.6} cross_seconds={:.6} sort_seconds={:.6} merge_seconds={:.6} reduce_seconds={:.6} finish_seconds={:.6} total_seconds={:.6}",
        left.z0,
        right.z1,
        total_nodes,
...
fn split_h2_attach_indices(events: &[H2AttachEvent]) -> (Vec<usize>, Vec<usize>) {
...
fn merge_h2_event_streams(
    ...
    let (left_outside, left_finite) = split_h2_attach_indices(left_attach);
    let (right_outside, right_finite) = split_h2_attach_indices(right_attach);
    let total = left_attach.len()
        + right_attach.len()
        + left_interface.len()
        + right_interface.len()
    ...
    while merged.len() < total {
        ...
        if cursor[0] < left_outside.len() {
            consider(0, 0, left_attach[left_outside[cursor[0]]].value);
        }
        if cursor[1] < right_outside.len() {
            consider(1, 0, right_attach[right_outside[cursor[1]]].value);
        }
        if cursor[2] < left_finite.len() {
            consider(2, 1, left_attach[left_finite[cursor[2]]].value);
        }
        if cursor[3] < right_finite.len() {
            consider(3, 1, right_attach[right_finite[cursor[3]]].value);
        }
        if cursor[4] < left_interface.len() {
            consider(4, 2, left_interface[cursor[4]].value);
        }
        if cursor[5] < right_interface.len() {
            consider(5, 2, right_interface[cursor[5]].value);
        }
        if cursor[6] < cross.len() {
            consider(6, 3, cross[cursor[6]].value);
        }

        let (_, _, stream) = best.expect("H2 ordered fan-in lost a non-empty source");
        let event = match stream {
            0 => {
                let source = left_attach[left_outside[cursor[0]]];
                cursor[0] += 1;
                H2Event {
                    value: source.value,
            ...
            1 => {
                let source = right_attach[right_outside[cursor[1]]];
                cursor[1] += 1;
                H2Event {
                    value: source.value,
                    kind: H2EventKind::Outside {
            ...
            2 => {
                let source = left_attach[left_finite[cursor[2]]];
                cursor[2] += 1;
                H2Event {
                    value: source.value,
            ...
            3 => {
                let source = right_attach[right_finite[cursor[3]]];
                cursor[3] += 1;
                H2Event {
                    value: source.value,
                    kind: H2EventKind::Attach {
            ...
            4 => {
                let source = left_interface[cursor[4]];
                cursor[4] += 1;
                H2Event {
                    value: source.value,
            ...
            5 => {
                let source = right_interface[cursor[5]];
                cursor[5] += 1;
                H2Event {
                    value: source.value,
                    kind: H2EventKind::Interface {
...
fn apply_h2_action(
    ...
    match action {
        H2Action::None => {}
        H2Action::Final(merge) => history.push_merge(merge),
...
fn combine_h2(
    left: HierarchicalSummary<SlabH2MergeTreeSummary>,
    right: HierarchicalSummary<SlabH2MergeTreeSummary>,
    connectivity: Connectivity,
    history: &mut H2FinalizedHistory,
    ...
    println!(
        "PROFILE_BRANCH_H2_HIER_COMBINE z0={} z1={} pair_nodes={} parent_interface_nodes={} parent_attach={} parent_interface={} cross_retained={} cross_candidates={} setup_seconds={:.6} cross_seconds={:.6} sort_seconds={:.6} merge_seconds={:.6} reduce_seconds={:.6} finish_seconds={:.6} total_seconds={:.6}",
        left.z0,
        right.z1,
        total_nodes,
        interface_node_count,
...
pub fn compute_h0_merge_tree_hierarchical_zslabs(
    ...
    for batch in ranges.chunks(leaf_workers) {
        ...
        let parallel_results: Vec<Result<(usize, usize, usize, SlabH0MergeTreeSummary, f64, f64)>> =
            batch
                .par_iter()
                .map(|&(slab_id, z0, z1)| {
                    ...
                    let summary = process_slab_h0_merge_tree(slab_id, &block, connectivity);
                    let compute_seconds = compute_start.elapsed().as_secs_f64();
                    Ok((slab_id, z0, z1, summary, read_seconds, compute_seconds))
                })
                .collect();
        ...
        for (_slab_id, z0, z1, mut summary, read_seconds, compute_seconds) in leaves {
            ...
            fan_in(
                ...
                &mut slots,
                |left, right| combine_h0(left, right, connectivity, &mut finalized_history),
                &mut combines,
                &mut max_pair_nodes,
    ...
    for right in remaining {
        ...
        root = combine_h0(root, right, connectivity, &mut finalized_history)?;
        root_combine_seconds += root_combine_start.elapsed().as_secs_f64();
        combines += 1;
    }

    debug_assert!(root.summary.local_merge_events.is_empty());
    let finalized_input_events = finalized_history.input_events;
    let contracted_zero_events = finalized_history.contracted_zero_events;
    let repaired_history_parent_refs = finalized_history.repaired_parent_refs;
...
pub fn compute_h2_merge_tree_hierarchical_zslabs(
    ...
    for batch in ranges.chunks(leaf_workers) {
        let leaf_batch_start = Instant::now();
        let parallel_results: Vec<Result<(usize, usize, usize, SlabH2MergeTreeSummary, f64, f64)>> =
            batch
                .par_iter()
                .map(|&(slab_id, z0, z1)| {
                    let read_start = Instant::now();
                    ...
                    let read_seconds = read_start.elapsed().as_secs_f64();
                    let compute_start = Instant::now();
                    let summary = process_slab_h2_merge_tree(
                        slab_id,
                        &block,
                        global_shape,
        ...
        for (_slab_id, z0, z1, mut summary, read_seconds, compute_seconds) in leaves {
            ...
            fan_in(
                ...
                &mut slots,
                |left, right| {
                    combine_h2(left, right, background_connectivity, &mut finalized_history)
                },
                &mut combines,
                &mut max_pair_nodes,
                |summary| summary.interface_node_count as usize,
    ...
    for right in remaining {
        ...
        let root_combine_start = Instant::now();
        root = combine_h2(root, right, background_connectivity, &mut finalized_history)?;
        root_combine_seconds += root_combine_start.elapsed().as_secs_f64();
        combines += 1;
    }
