#!/usr/bin/env python3
"""Exact U16 H2 persistence equivalence: one-hop vs two-hop parent shortcut."""
...
def compare(depth: int, reference: list[Interval], candidate: list[Interval], require_order: bool) -> None:
    ...
    if a != b:
        ...
        raise RuntimeError(
            f"FAIL H2 d={depth}: one-hop/two-hop interval multisets differ\n"
            f"missing from two-hop: {missing}\nextra in two-hop: {extra}"
        )
    if require_order and reference != candidate:
        ...
        raise RuntimeError(
            ...
            f"one-hop={reference[first]} two-hop={candidate[first]}"
        )
...
def main() -> int:
    ...
    with tempfile.TemporaryDirectory(prefix="u16_h2_two_hop_equiv_") as td_raw:
        td = Path(td_raw)
        for depth in args.slab_depths:
            ref_path = td / f"h2_d{depth}_one_hop.csv"
            cand_path = td / f"h2_d{depth}_two_hop.csv"
            ...
            cand_log = run(base + [str(cand_path)] + extra + ["--neighbor-root-check", "parent-two-hop"])
            ...
            if "neighbor_root_check=parent-two-hop" not in cand_log:
                raise RuntimeError(f"FAIL H2 d={depth}: two-hop run did not report parent-two-hop")
            ...
            print(
                f"PASS H2 d={depth}: {len(reference)} exact intervals; two-hop matches one-hop "
                f"reference ({order_note}); sha256={digest}"
            )

    print(
        f"PASS H2 two-hop parent-shortcut gate across slab depths {','.join(map(str, args.slab_depths))}: "
