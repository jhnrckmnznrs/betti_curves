#!/usr/bin/env python3
"""Exact scalar persistence check for parent-rank versus packed local UF layout."""
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix='betti_uf_layout_equiv_') as td:
        root = Path(td)
        for dim in a.modes:
            ...
            reference, _ = run(a.binary, a.input, root / f'{dim}_parent_rank', a.slab_depth,
                               a.foreground_connectivity, f'{dim}-scalar-stream',
                               fixed + ['--uf-layout', 'parent-rank'])
            packed, _ = run(a.binary, a.input, root / f'{dim}_packed', a.slab_depth,
                            a.foreground_connectivity, f'{dim}-scalar-stream',
                            fixed + ['--uf-layout', 'packed'])
            check(f'{dim} parent-rank vs in-memory', oracle, reference)
            check(f'{dim} packed vs in-memory', oracle, packed)
            check(f'{dim} packed vs parent-rank', reference, packed)
    print('PASS: packed local UF layout preserves exact scalar persistence')
