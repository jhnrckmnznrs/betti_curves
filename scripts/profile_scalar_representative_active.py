#!/usr/bin/env python3
"""Balanced same-binary profiler for representative active recheck versus trust-pruner."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output", type=Path, default=Path("scalar_representative_active_profile.csv"))
...
def run_once(args: argparse.Namespace, check: str, mode: str, repeat: int, sequence: int,
             root: Path, record: bool) -> dict[str, object] | None:
    tag = "warmup" if not record else f"r{repeat:03d}"
    run_dir = root / f"{sequence:04d}_{mode}_{check}_{tag}"
    ...
    command = [
        ...
        "--representative-active-check", check,
    ]
    ...
    if completed.returncode != 0:
        raise RuntimeError(f"{mode} {check} failed; see {log_path}")
    if not intervals.is_file():
        raise RuntimeError(f"{mode} {check} produced no interval CSV")
    if not record:
    ...
    row: dict[str, object] = {
        "mode": mode, "representative_active_check": check, "repeat": repeat,
...
def main() -> None:
    ...
    sequence = 0
    with tempfile.TemporaryDirectory(prefix="betti_representative_active_profile_") as temp:
        root = Path(temp)
        for mode in args.modes:
            for _ in range(args.warmup):
                for check in ("recheck", "trust-pruner"):
                    run_once(args, check, mode, -1, sequence, root, False)
                    sequence += 1
            for repeat in range(args.repeats):
                order = ("recheck", "trust-pruner") if repeat % 2 == 0 else ("trust-pruner", "recheck")
                for check in order:
                    row = run_once(args, check, mode, repeat, sequence, root, True)
                    sequence += 1
    ...
    if bad:
        raise RuntimeError(f"representative-active outputs disagree: {bad}")

    fields = [
        "mode", "representative_active_check", "repeat", "run_sequence", "slab_depth",
    ...
    print("\nMedian representative-active ablation")
    print("mode\tcheck\twall\tprepare\tsweep\treduce\tRSS_MiB")
    groups: dict[tuple[str, str], list[dict[str, object]]] = defaultdict(list)
    for row in rows:
        groups[(str(row["mode"]), str(row["representative_active_check"]))].append(row)
    ...
    for mode in args.modes:
        recheck = groups[(mode, "recheck")]
        trust = groups[(mode, "trust-pruner")]
        rw = median(recheck, "wall_seconds")
        tw = median(trust, "wall_seconds")
        rs = median(recheck, "local_sweep_seconds")
        ts = median(trust, "local_sweep_seconds")
        print(f"speedup {mode} wall: {rw / tw:.4f}x; local sweep: {rs / ts:.4f}x")
    print(f"PASS: exact persistence agreement; wrote {args.output}")
