#!/usr/bin/env python3
"""Balanced same-binary profiler for separate active array versus parent-sentinel active state."""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--output", type=Path, default=Path("scalar_active_state_profile.csv"))
...
def run_once(args: argparse.Namespace, state: str, mode: str, repeat: int, sequence: int,
             root: Path, record: bool) -> dict[str, object] | None:
    tag = "warmup" if not record else f"r{repeat:03d}"
    run_dir = root / f"{sequence:04d}_{mode}_{state}_{tag}"
    ...
    if completed.returncode != 0:
        raise RuntimeError(f"{mode} {state} failed; see {log_path}")
    if not intervals.is_file():
        raise RuntimeError(f"{mode} {state} produced no interval CSV")
    if not record:
    ...
    row: dict[str, object] = {
        "mode": mode, "active_state": state, "repeat": repeat,
...
def main() -> None:
    ...
    sequence = 0
    with tempfile.TemporaryDirectory(prefix="betti_active_state_profile_") as temp:
        root = Path(temp)
        for mode in args.modes:
            for _ in range(args.warmup):
                for state in ("separate", "parent-sentinel"):
                    run_once(args, state, mode, -1, sequence, root, False)
                    sequence += 1
            for repeat in range(args.repeats):
                order = ("separate", "parent-sentinel") if repeat % 2 == 0 else ("parent-sentinel", "separate")
                for state in order:
                    row = run_once(args, state, mode, repeat, sequence, root, True)
                    sequence += 1
    ...
    if bad:
        raise RuntimeError(f"active-state outputs disagree: {bad}")

    fields = [
        "mode", "active_state", "repeat", "run_sequence", "slab_depth",
    ...
    print("\nMedian active-state ablation")
    print("mode\tactive_state\twall\tprepare\tsweep\treduce\tRSS_MiB")
    groups: dict[tuple[str, str], list[dict[str, object]]] = defaultdict(list)
    for row in rows:
        groups[(str(row["mode"]), str(row["active_state"]))].append(row)
    ...
    for mode in args.modes:
        separate = groups[(mode, "separate")]
        sentinel = groups[(mode, "parent-sentinel")]
        sw = median(separate, "wall_seconds")
        pw = median(sentinel, "wall_seconds")
        ss = median(separate, "local_sweep_seconds")
        ps = median(sentinel, "local_sweep_seconds")
        sr = median(separate, "max_rss_kb")
        pr = median(sentinel, "max_rss_kb")
        print(f"speedup {mode} wall: {sw / pw:.4f}x; local sweep: {ss / ps:.4f}x; RSS change: {(pr/sr-1)*100:.2f}%")
    print(f"PASS: exact persistence agreement across active-state modes; wrote {args.output}")
