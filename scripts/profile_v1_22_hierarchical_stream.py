...
def parse_args():
    ...
    p.add_argument("--output", type=Path, default=Path("cx09t1_v1_22_hierarchical_stream.csv"))
...
def run_once(a, root, seq, variant, slab, fixture, repeat):
    ...
    if variant == "hier-reference":
        ...
        for k, v in kv_line(cp.stdout, "PROFILE_GLOBAL_H0_STATE scalar_h0_hierarchical_stream ").items():
            row[f"root_{k}"] = v
        for k, v in kv_line(cp.stdout, "PROFILE_H0_HIER_STREAM_FINAL ").items():
            row[f"hier_stream_final_{k}"] = v
    else:
...
def main():
    ...
    with tempfile.TemporaryDirectory(prefix="betti_v122_profile_") as td:
        root = Path(td)
    ...
    for (variant, slab), rs in sorted(grouped.items()):
        ...
        for key in (
            ...
            "hier_stream_root_materialized", "hier_stream_final_pair_nodes",
            "hier_stream_final_pair_state_bytes", "hier_stream_final_root_materialized",
    ...
    print("PASS: all v1.22 hierarchy profile variants are persistence-identical")
    for s in summary:
        print(
